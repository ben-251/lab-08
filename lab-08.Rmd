...
```{r filter-rain-nas}
# the average "na-ness" of raintomorrow is almost zero, but not zero, so there are nas.
...
```{r filter-for-portland}
weather_Portland <- weather_noNA %>%
  filter(Location == "Portland")

weather_Portland_nos <- weather_Portland %>%
  mutate(
    RainToday = case_when(
       RainToday == "Yes" ~ 1,
       RainToday == "No" ~ 0
...
```{r lin-reg}
rain_fit_1 <- linear_reg() %>%
  set_engine("lm") %>%
  fit(RainTomorrow ~ RainToday, data = weather_Portland_nos)
tidy(rain_fit)
```
let M = probability it rains tomorrow, T = probability it rains today, then:
M = 0.309T + 0.254 
meaning if it doesn't rain today, there's a 0.254 chance of rain. 
if it does, theres a 0.55 chance of rain (such little rain!)
...
```{r probability-line}
weather_Portland %>%
  ggplot(mapping = aes(x=RainTomorrow,y=RainToday)) + 
  geom_point() +
...
the 4 combinations of expected vs actual results, based on whether it rained previous day or not:
raintoday and raintomorrow
not rain today and not rain tomorrow
raintoday but not rain tomorrow
not rain today but rain tomorrow

we filtered to make sure raintomorrow was never na, but didn't do the same for rain today?
